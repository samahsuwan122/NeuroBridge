...
export function useProviderSelf(){
 ...
 const update=async(_patch:ProviderSelfPatch):Promise<Provider>=>{throw new Error("Provider profile editing is unavailable.")};
 const uploadPhoto=async(_file:File):Promise<Provider>=>{throw new Error("Provider photo uploads are unavailable.")};
 return{provider,loading,error,load,update,uploadPhoto,editingAvailable:false};
}
